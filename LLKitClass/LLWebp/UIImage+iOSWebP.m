...
@implementation UIImage (iOSWebP)
...
+ (void)hp_animatedWebpGIFWithData:(NSData *)data imageBlock:(void (^)(UIImage *image))imageBlock {
    if (!data) {
        imageBlock(nil);
        return;
    }
    ...
    if (count <= 1) {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            UIImage *imageO  = [UIImage imageWithCGImage:image scale:[UIScreen mainScreen].scale orientation:UIImageOrientationUp];
            if (i == 0) {
                imageBlock(imageO);
            }
            NSData *imageDataW = [UIImage imageToWebP:imageO quality:90];
            UIImage *iamgeW =  [UIImage imageWithWebPData:imageDataW];
            
            [images addObject:iamgeW];
...
+ (UIImage *)hp_animatedWebpGIFWithData:(NSData *)data {
    if (!data) {
        return nil;
    }
