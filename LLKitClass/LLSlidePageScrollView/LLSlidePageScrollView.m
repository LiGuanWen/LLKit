//
//  LLSlidePageScrollView.m
//  LLSlidePageScrollViewDemo


#import "LLSlidePageScrollView.h"
#import "UIScrollView+LLSwizzle.h"

@interface LLBasePageTabBar ()
@property (nonatomic, weak) id<LLBasePageTabBarPrivateDelegate> praviteDelegate;
@end

@interface LLSlidePageScrollView ()<UIScrollViewDelegate,LLBasePageTabBarPrivateDelegate>{
    struct {
...
@property (nonatomic, strong) NSLayoutConstraint *headerContenLLConstraint;
...
@property (nonatomic, assign) CGFloat pageScrollViewOffseLL;

@end

@implementation LLSlidePageScrollView
...
- (void)setDelegate:(id<LLSlidePageScrollViewDelegate>)delegate
{
    _delegate = delegate;
...
- (void)layoutHeaderContentView
{
    ...
    _headerContenLLConstraint = [NSLayoutConstraint constraintWithItem:_headerContentView attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeTop multiplier:1 constant:0];
    ...
    [self addConstraint:_headerContenLLConstraint];
...
- (void)changeAllPageScrollViewOffseLL:(CGFloat)offseLL isOnTop:(BOOL)isOnTop
{
    [_pageViewArray enumerateObjectsUsingBlock:^(UIScrollView *pageScrollView, NSUInteger idx, BOOL *stop) {
        if (idx != _curPageIndex && !(isOnTop && pageScrollView.contentOffset.y > offseLL)) {
            [pageScrollView setContentOffset:CGPointMake(pageScrollView.contentOffset.x, offseLL)];
        }
    }];
}
...
- (void)pageScrollViewDidScroll:(UIScrollView *)pageScrollView changeOtherPageViews:(BOOL)isNeedChange
{
    ...
    CGFloat offseLL = pageScrollView.contentOffset.y;
    if (offseLL <= -_headerContentViewHeight) {
        // headerContentView full show
        if (_headerContenLLConstraint.constant != 0) {
            _headerContenLLConstraint.constant = 0;
            if (_delegateFlags.pageTabBarScrollOffset) {
                [_delegate slidePageScrollView:self pageTabBarScrollOffset:offseLL state:LLPageTabBarStateStopOnButtom];
            }
        }
        if (_parallaxHeaderEffect) {
            _headerContentHeightConstraint.constant = -offseLL;
        }
        
        if (isNeedChange) {
            [self changeAllPageScrollViewOffseLL:-_headerContentViewHeight isOnTop:NO];
        }
    }else if (offseLL < -pageTabBarHieght - pageTabBarIsStopOnTop) {
        ...
        _headerContenLLConstraint.constant = -(offseLL+_headerContentViewHeight);
        
        if (_delegateFlags.pageTabBarScrollOffset) {
            [_delegate slidePageScrollView:self pageTabBarScrollOffset:offseLL state:LLPageTabBarStateScrolling];
        }
        
        if (isNeedChange) {
            [self changeAllPageScrollViewOffseLL:pageScrollView.contentOffset.y isOnTop:NO];
        }
        ...
        if (_headerContenLLConstraint.constant != -_headerContentViewHeight+pageTabBarHieght + pageTabBarIsStopOnTop) {
            _headerContenLLConstraint.constant = -_headerContentViewHeight+pageTabBarHieght + pageTabBarIsStopOnTop;

            if (_delegateFlags.pageTabBarScrollOffset) {
                [_delegate slidePageScrollView:self pageTabBarScrollOffset:offseLL state:LLPageTabBarStateStopOnTop];
            }
        }
        
        if (isNeedChange) {
            [self changeAllPageScrollViewOffseLL:-pageTabBarHieght-pageTabBarIsStopOnTop isOnTop:YES];
        }
    }
}
...
-(void)dealloc
{
    //[self resetProperLLs];
    [self addPageViewKeyPathOffsetWithOldIndex:_curPageIndex newIndex:-1];
    NSLog(@"LLSlidePageScrollView dealloc");
}
