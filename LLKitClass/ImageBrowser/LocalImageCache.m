...
static const NSInteger kDefaultCacheMaxCacheAge = INT_MAX; // 永不过期
// PNG signature bytes and data (below)
static unsigned char kPNGSignatureBytes[8] = {0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A};
static NSData *kPNGSignatureData = nil;

BOOL ImageDataHasPNGPreffix_local(NSData *data);
...
- (id)init {
    return [self initWithNamespace:@"Local"];
}

- (id)initWithNamespace:(NSString *)ns {
    if ((self = [super init])) {
        NSString *fullNamespace = [@"com.ppche.PPAuto" stringByAppendingString:ns];
        
        // Create IO serial queue
        _ioQueue = dispatch_queue_create("com.ppche.PPAuto", DISPATCH_QUEUE_SERIAL);
        ...
        // Init the disk cache 缓存在Document目录中，不会被系统删除
...
#pragma mark LocalImageCache (private)
        ...
        dispatch_async(self.ioQueue, ^{
            ...
            if (image && (recalculate || !data)) {
                ...
                if ([imageData length] >= [kPNGSignatureData length]) {
                    imageIsPng = ImageDataHasPNGPreffix_local(imageData);
                }
...
- (void)storeJPEGImage:(UIImage *)image imageData:(NSData *)imageData forKey:(NSString *)key compressionFactor:(float)factor {
    [self storeJPEGImage:image recalculateFromImage:YES imageData:imageData forKey:key toDisk:YES compressionFactor:factor];
}
...
- (void)storeJPEGImage:(UIImage *)image recalculateFromImage:(BOOL)recalculate imageData:(NSData *)imageData forKey:(NSString *)key toDisk:(BOOL)toDisk compressionFactor:(float)factor {
    ...
    if (toDisk) {
        dispatch_async(self.ioQueue, ^{
            ...
            if (image && (recalculate || !data)) {
#if TARGET_OS_IPHONE
                data = UIImageJPEGRepresentation(image, (CGFloat)factor);
