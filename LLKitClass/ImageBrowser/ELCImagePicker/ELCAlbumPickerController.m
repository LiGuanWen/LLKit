        ...
        // Group enumerator Block
            ...
            void (^assetGroupEnumberatorFailure)(NSError *) = ^(NSError *error) {
                ...
                if ([ALAssetsLibrary authorizationStatus] == ALAuthorizationStatusDenied) {
                    UIAlertView * alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"请在iPhone的“设置”-“隐私”-“照片”选项中，允许“小鱼理车”访问你的手机相册。" delegate:nil cancelButtonTitle:@"我知道了" otherButtonTitles:nil];
                    [alert show];
                } else {
                    NSString *errorMessage = [NSString stringWithFormat:@"打开相册错误: %@ - %@", [error localizedDescription], [error localizedRecoverySuggestion]];
...
{
	[self.tableView reloadData];
	[self.navigationItem setTitle:NSLocalizedString(@"选择一个相册", nil)];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    float factor = [UIScreen mainScreen].bounds.size.width/320.0;

    image = [self resize:image to:CGSizeMake(78 * factor, 78 * factor)];
